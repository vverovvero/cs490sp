...
class Scene(object):
    ...
    #add a light
    def add_light(self, type, point, color, toPoint, angle, fallOffAngle, radius):
    	light = {'type': type, 'point': point, 'color': color, 'toPoint': toPoint, 'angle': angle, 'fallOffAngle': fallOffAngle, 'radius': radius}
...
def writeB(scene, b_name): 
	...
	for i in range(scene.n_lights):
		...
		t = BitArray()
		t = bitstring.pack("2*int:32, 12*float:32", 2,
			light['type'], 
			...
			light['fallOffAngle'],
			light['radius'])
		print t.unpack("2*int:32, 12*float:32")
		s = s + t
	...
	def light(self, scene, attributes):
		...
		for attribute in attributes:
			...
			if function == "Type":
				...
				light_args["Radius"] = args_list[0]
			else:
				print "Invalid Light attribute: " + function
		scene.add_light(light_args["Type"], light_args["Point"], light_args["Color"], light_args["ToPoint"], light_args["Angle"], light_args["FallOffAngle"], light_args["Radius"])
